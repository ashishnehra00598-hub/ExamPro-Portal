...
export default function SuperDashboard() {
  ...
  useEffect(() => {
    async function loadMasterData() {
      ...
      // Tests
      const { data: testsData } = await supabase.from('tests').select('*').order('id', { ascending: true });
      ...
      // Global Scores
      const { data: globalScores } = await supabase.from('test_results').select('*');
      if (globalScores) setAllScores(globalScores);

      // Alerts
      const { data: examsData } = await supabase.from('exam_alerts').select('*');
      if (examsData) setExamAlerts(examsData);

      // Materials
      const { data: matData } = await supabase.from('study_materials').select('*').order('id', { ascending: false });
  ...
  const calculateRealAccuracy = () => {
    ...
    let totalMaxScore = 0;
    results.forEach((r) => {
      totalScoreObtained += r.score;
      totalMaxScore += (r.total_questions * 2);
    });
  ...
  // Leaderboard (विद स्टूडेंट रियल नेम)
  const calculateOverallLeaderboard = () => {
    ...
    allScores.forEach((row) => {
      if (!userTotals[row.user_email]) {
        userTotals[row.user_email] = {
          totalScore: 0,
          testsGiven: 0,
          name: row.user_name || row.user_email.split('@')[0],
        };
      }
      userTotals[row.user_email].totalScore += row.score;
      userTotals[row.user_email].testsGiven += 1;
    });
        ...
        <div className="flex items-center gap-4">
          {user?.email === ADMIN_EMAIL && (
            <button
              ...
              className="text-xs bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 px-3 py-1.5 rounded-xl font-bold transition"
            >
              ⚙️ Admin Panel
            </button>
          )}
          <button
            ...
            className="text-xs bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 px-3 py-1.5 rounded-xl font-bold transition"
          >
            Logout
          </button>
        </div>
      ...
      <main className="max-w-6xl w-full mx-auto p-6 md:p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-400 font-semibold uppercase">Real Study Streak</p>
              <p className="text-2xl font-black text-amber-400">{realStreak} {realStreak === 1 ? 'Day' : 'Days'} Active</p>
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-400 font-semibold uppercase">कुल अर्जित स्कोर</p>
              ...
              {EXAM_CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`px-5 py-3 rounded-2xl text-xs font-bold whitespace-nowrap transition border ${
                      isActive
                ...
                >
                  ...
                  <div className="pt-3 border-t border-slate-800 space-y-3">
                    ...
                    <button
                      ...
                      className="w-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold py-3 rounded-xl transition shadow-lg"
                    >
                      Start Test Paper
                    </button>
                  </div>
        ...
        {/* Dual Leaderboard (Real Names Shown) */}
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
            <div className="flex justify-between items-center">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-700 text-xs font-bold text-indigo-300 rounded-xl px-3 py-1.5 outline-none"
              >
            ...
            <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
              {currentLeaderboard.map((item: any, idx: number) => {
                ...
                const displayName = item.name || item.user_name || email.split('@')[0];
                ...
                <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-2xl space-y-1">
                  ...
                  <p className="text-[11px] text-slate-400">
                    {realAccuracy >= 70
                      ...
                      : realAccuracy >= 40
                      ? 'औसत प्रदर्शन। गलत होने वाले सवालों का रिवीजन करें।'
